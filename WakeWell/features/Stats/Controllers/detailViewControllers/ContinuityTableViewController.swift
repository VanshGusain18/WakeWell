...
class ContinuityTableViewController: BaseMetricTableViewController {

    private var data: [ContinuityData] = []

    override func loadData(for range: StatsTimeRange) {
        data = SleepContinuityAnalyzer.getData(for: range)
    }
    ...
    override func buildCell(for section: SectionType, range: StatsTimeRange,
        ...
        case .header:
            ...
            cell.configure(
                title: "Continuity",
                description: "How uninterrupted your sleep is throughout the night."
            )
            return cell

        case .trend:
            let cell = dequeueLineChart(tableView, indexPath: indexPath)
            let chartData = SleepContinuityAnalyzer.trendChartData(from: data)
        ...
        case .fragmentation:
            ...
            let chartData = SleepContinuityAnalyzer.fragmentationChartData(from: data)
            cell.configure(title: chartData.title,
                           dataSets: chartData.dataSets,
                           xAxisLabels: chartData.xAxisLabels)
            return cell
        ...
        case .info:
            ...
            cell.configure(title: "About Sleep Continuity",
