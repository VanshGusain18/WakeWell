...
class CalmnessTableViewController: BaseMetricTableViewController {

    private var data: [CalmnessData] = []

    override func loadData(for range: StatsTimeRange) {
        data = SleepCalmnessAnalyzer.getData(for: range)
    }
    ...
    override func buildCell(for section: SectionType, range: StatsTimeRange,
        ...
        case .header:
            ...
            cell.configure(
                title: "Calmness",
                description: "Measures restlessness during sleep using movement and stress signals."
            )
            return cell

        case .trend:
            let cell = dequeueLineChart(tableView, indexPath: indexPath)
            let chartData = SleepCalmnessAnalyzer.trendChartData(from: data)
