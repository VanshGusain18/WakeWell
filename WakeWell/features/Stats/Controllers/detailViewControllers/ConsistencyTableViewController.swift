...
class ConsistencyTableViewController: BaseMetricTableViewController {

    private var data: [SleepConsistencyData] = []
    ...
    override func buildCell(for section: SectionType, range: StatsTimeRange,
        ...
        case .header:
            ...
            cell.configure(
                title: "Consistency",
                description: "Tracks how regular your sleep schedule is over the \(range.title.lowercased())."
            )
            return cell

        case .trend:
            let cell = dequeueLineChart(tableView, indexPath: indexPath)
            let chartData = SleepConsistencyAnalyzer.trendChartData(from: data)
        ...
        case .fragmentation:
            ...
            let chartData = SleepConsistencyAnalyzer.timingChartData(from: data)
            cell.configure(title: chartData.title,
                           dataSets: chartData.dataSets,
                           xAxisLabels: chartData.xAxisLabels)
            return cell
        ...
        case .info:
            ...
            cell.configure(title: "About Sleep Consistency",
                           description: SleepConsistencyAnalyzer.consistencyInfo())
            return cell
        }
    }
