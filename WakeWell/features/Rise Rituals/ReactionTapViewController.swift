...
final class ReactionTapViewController: RoutineActivityViewController {
    ...
    private func showTarget() {
        ...
        let step = CGFloat((attempts % 7) + 1) / 8
        let alternateStep = CGFloat(((attempts * 3) % 7) + 1) / 8
        let x = 20 + (maxX - 20) * step
        let y = 20 + (maxY - 20) * alternateStep
        targetButton.frame.origin = CGPoint(x: x, y: y)
